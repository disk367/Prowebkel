...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
  ...
  if(!$d['title']||!$d['company']||!$d['location']||!$d['category']||!$d['description']||!$d['requirements']){
    ...
    $st = $conn->prepare("INSERT INTO jobs(title,company,location,type,category,description,requirements,salary_min,salary_max,created_by)VALUES(?,?,?,?,?,?,?,?,?,?)");
    $st->bind_param('ssssssssii',$d['title'],$d['company'],$d['location'],$d['type'],$d['category'],$d['description'],$d['requirements'],$smin,$smax,$_SESSION['user_id']);
    if($st->execute()){
      $_SESSION['flash'] = "Lowongan \"{$d['title']}\" berhasil ditambahkan!";
      redirect('index.php');
    } else { $err='Gagal menyimpan, coba lagi.'; }
  }
}
$d = $_POST ?? ['type'=>'Full-time','category'=>''];
?>
<!DOCTYPE html>
<html lang="id">
<head>
<meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>Tambah Lowongan — ZenirWork</title>
...
<div class="dash-wrap">
  <aside class="sidebar">
    ...
    <a href="create.php" class="sb-link on"><i class="bi bi-plus-circle"></i> Tambah Lowongan</a>
  ...
  <main class="dash-main">
    <div style="display:flex;align-items:center;gap:12px;margin-bottom:22px">
      ...
      <div>
        <h1 style="font-family:'Plus Jakarta Sans',sans-serif;font-size:20px;font-weight:700;color:#fff">Tambah Lowongan Baru</h1>
        <p style="font-size:13px;color:rgba(255,255,255,.35)">Isi informasi lowongan dengan lengkap</p>
      </div>
    </div>
    ...
    <form method="POST" novalidate>
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:20px">
        <div class="form-card">
          ...
          <div class="fg2">
            <label>Judul Posisi <span style="color:#ef4444">*</span></label>
            <input type="text" name="title" class="fi2" placeholder="cth: Senior UI/UX Designer" value="<?= e($d['title']??'') ?>">
          </div>
          <div class="fg2">
            <label>Nama Perusahaan <span style="color:#ef4444">*</span></label>
            <input type="text" name="company" class="fi2" placeholder="cth: Tokopedia" value="<?= e($d['company']??'') ?>">
          </div>
          <div class="fg2">
            <label>Lokasi <span style="color:#ef4444">*</span></label>
            <input type="text" name="location" class="fi2" placeholder="cth: Jakarta / Remote" value="<?= e($d['location']??'') ?>">
          </div>
          <div class="form-row">
            <div class="fg2">
            ...
            <div class="fg2">
              ...
              <select name="category" class="fi2">
                ...
                <option value="<?= $k ?>" <?= ($d['category']??'')==$k?'selected':'' ?>><?= $k ?></option>
                <?php endforeach; ?>
              </select>
            </div>
          </div>
          <div class="form-row">
            <div class="fg2">
              <label>Gaji Min (Rp) <span style="color:#ef4444">*</span></label>
              <input type="number" name="salary_min" class="fi2" placeholder="5000000" value="<?= e($d['salary_min']??'') ?>">
            </div>
            <div class="fg2">
              <label>Gaji Max (Rp) <span style="color:#ef4444">*</span></label>
              <input type="number" name="salary_max" class="fi2" placeholder="9000000" value="<?= e($d['salary_max']??'') ?>">
            </div>
          </div>
        </div>
        ...
        <div class="form-card">
          ...
          <div class="fg2">
            ...
            <textarea name="description" class="fi2 ta2" placeholder="Jelaskan tanggung jawab dan gambaran pekerjaan..."><?= e($d['description']??'') ?></textarea>
          </div>
          <div class="fg2">
            <label>Persyaratan <span style="color:#ef4444">*</span></label>
            <textarea name="requirements" class="fi2 ta2" style="min-height:130px" placeholder="Satu syarat per baris&#10;cth: Mahir Figma&#10;Pengalaman min. 1 tahun"><?= e($d['requirements']??'') ?></textarea>
          </div>
        </div>
      </div>
      ...
      <div style="display:flex;gap:10px;margin-top:20px">
        <button type="submit" class="btn-sm" style="padding:10px 24px;font-size:14px">
          <i class="bi bi-check-lg"></i> Simpan Lowongan
        </button>
